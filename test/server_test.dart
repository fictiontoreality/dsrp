...
void main() {
  ...
  group('Server object tests', () {
    ...
    group('verifySession tests', () {
      final userSessionKeyVerifier = Uint8List.fromList([112, 68, 29, 211, 34, 202, 76, 147, 231, 195, 23, 181, 197, 114, 154, 72, 227, 238, 18, 235, 59, 22, 61, 155, 143, 248, 68, 202, 125, 239, 103, 176]);

      test('verification passes and returns correct server verifier', () async {
        final expectedServerSessionKeyVerifier = Uint8List.fromList([11, 114, 210, 88, 9, 28, 238, 168, 77, 158, 32, 102, 43, 253, 72, 248, 164, 73, 231, 6, 105, 181, 76, 144, 203, 230, 220, 199, 102, 215, 28, 255]);
      ...
      test('verification fails for incorrect user verifier', () async {
        ...
        expect(
          server.verifySession(
            ephemeralUserPublicKey: userPublicKey,
            userSessionKeyVerifier: Uint8List.fromList([...userSessionKeyVerifier, 1, 2, 3, 4]),
          ),
