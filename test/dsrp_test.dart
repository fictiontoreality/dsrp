...
void main() {
  authenticate(User user, List<int> verifierKey, List<int> salt) async {
  ...
  group('end to end integration tests', () {
      test('full SRP workflow', () async {
          ...
          // Authentication.
          await authenticate(user, saltedVerificationKey.key, saltedVerificationKey.salt);
      });

      test('authenticate with pre-existing verifier key and salt', () async {
          ...
          // Authentication.
          final user = User(userId: username, password: password);
          await authenticate(user, verifierKey, salt);
      });
  });
