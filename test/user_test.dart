...
void main() {
  ...
  group('processChallenge tests', () {
      setUp(() {
         user.startAuthentication(ephemeralPrivateUserKeyBytes: ephemeralPrivateUserKey);
      });

      test('session key and its verifier matches pysrp', () async {
          ...
          final sessionKeyVerifier = await user.processChallenge(salt, ephemeralPublicServerKey);
  ...
  group('verifySession tests', () {
      ...
      setUp(() async {
          user.startAuthentication(ephemeralPrivateUserKeyBytes: ephemeralPrivateUserKey);
          await user.processChallenge(salt, ephemeralPublicServerKey);
      });
