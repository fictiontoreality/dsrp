...
void main() {
  ...
  group('generateSaltedVerificationKey tests', () {
      test('generated verification key is same as pysrp', () async {
          ...
          final verifierKey = await user.createSaltedVerificationKey(salt: salt);
  ...
  group('startAuthentication tests', () {
      test('epehemeral public user key matches pysrp', () {
          ...
          final startAuthData = user.startAuthentication(userPrivateKeyBytes: userPrivateKey);
          expect(startAuthData.userId, username);
          expect(startAuthData.ephemeralUserPublicKey, expectedUserPublicKey);
      });
  });
