...
class User {
  ...
  StartAuthenticationData startAuthentication({List<int>? userPrivateKeyBytes}) {
    ...
    userPrivateKey = convertByteListToBigInt(userPrivateKeyBytes);
    // A = g^a
    final userPublicKey = generator.modPow(userPrivateKey!, safePrime);
    userPublicKeyBytes = convertBigIntToByteList(userPublicKey);
  ...
  Future<List<int>> processChallenge(
    ...
    final hashedGenerator =
        convertByteListToBigInt(await _rfc5054Hash([convertBigIntToByteList(generator)]));
    // H(I)
  ...
  Future<BigInt> _derivePrivateKey(List<int> salt) async {
    ...
    final privateKey = convertByteListToBigInt(privateKeyHash.bytes);
    return privateKey;
  }
  ...
  Future<List<int>> _deriveSessionKey(
    ...
    final randomScramblingParameter = convertByteListToBigInt(
        await _rfc5054Hash([userPublicKeyBytes!, serverPublicKeyBytes]));
    if (randomScramblingParameter == BigInt.zero) {
      throw 'Abort authentication! Random scrambling parameter (u = H(A,B)) is zero.';
    }
    // k = H(N,g)
    final multiplierParameter = convertByteListToBigInt(await _rfc5054Hash(
        [convertBigIntToByteList(safePrime), convertBigIntToByteList(generator)]));
    // Notated 'B'.
    final serverPublicKey = convertByteListToBigInt(serverPublicKeyBytes);
    ...
    sessionKey = (await hashAlgorithm.hash(convertBigIntToByteList(secret))).bytes;
    return sessionKey!;
  }

  Future<List<int>> _rfc5054Hash(List<List<int>> byteLists) async {
    var bytes = <int>[];
    //OPTIMIZE: Make this a class field.
    final safePrimeBytes = convertBigIntToByteList(safePrime);
    for (var byteList in byteLists) {
      // RFC5054 require left-padding pre-concatenated bytes with zeros if bytes
      // length less than that of the safe prime.
      //OPTIMIZE: More efficient way to generate zeros?
      bytes += List<int>.generate(safePrimeBytes.length - byteList.length, (index) => 0);
      bytes += byteList;
    }
