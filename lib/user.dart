...
import 'defaults.dart' show defaultGenerator, defaultHashAlgorithm, defaultSafePrime;
...
class User {
  ...
  Future<List<int>> processChallenge(
    List<int> salt, List<int> ephemeralServerPublicKey
  ) async {
    sessionKey = await _deriveSessionKey(salt, ephemeralServerPublicKey);
    // H(N)
  ...
  verifySession(List<int> serverSessionKeyVerifier) async {
    ...
    final expectedServerSessionKeyVerifier = (await hashAlgorithm.hash(
        userPublicKeyBytes! + sessionKeyVerifier! + sessionKey!
    )).bytes;
  ...
  Future<List<int>> _deriveSessionKey(
    ...
    final sessionKey = (await hashAlgorithm.hash(secret.toByteList())).bytes;
    return sessionKey;
  }
