...
class User {
  ...
  Future<List<int>> processChallenge(
    ...
    final sessionKey = await _deriveSessionKey(salt, ephemeralPublicServerKeyBytes);
    // H(N)
    final hashedSafePrime = convertByteListToBigInt(
        (await hashAlgorithm.hash(convertBigIntToByteList(safePrime))).bytes);
    // H(g)
    final hashedGenerator = convertByteListToBigInt(await _rfc5054Hash(
        [convertBigIntToByteList(generator)]
    ));
    // H(I)
  ...
  verifySession(List<int> serverSessionKeyVerifier) async {
    // M2 = H(A, M, K)
    final expectedServerSessionKeyVerifier = (await hashAlgorithm.hash(
      ephemeralPublicUserKeyBytes! + sessionKeyVerifier! + sessionKey!
    )).bytes;
  ...
  Future<List<int>> _deriveSessionKey(
    List<int> salt, List<int> ephemeralPublicServerKeyBytes
  ) async {
    // x
    final privateKey = await _derivePrivateKey(salt);
    // v = g^x
    final verifierKey = _deriveVerificationKey(privateKey);
    // u = H(A,B)
    final randomScramblingParameter = convertByteListToBigInt(await _rfc5054Hash(
        [ephemeralPublicUserKeyBytes!, ephemeralPublicServerKeyBytes]
    ));
    ...
    if (ephemeralPublicServerKey % safePrime == BigInt.zero) {
      throw 'Abort authentication! Ephemeral public server key is invalid (B % N == 0).';
    }
    // B - k(g^x) = B - kv
    final firstTerm = ephemeralPublicServerKey - multiplierParameter * verifierKey;
    // a + ux
    final secondTerm = ephemeralPrivateUserKey! + randomScramblingParameter * privateKey;
    // S = (B - kv) ^ (a + ux)
